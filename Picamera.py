...
for ID in range(11,15):
	...
	dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, ID, ADDR_PRO_MOVE_SPEED , 170)
	if dxl_comm_result != COMM_SUCCESS:
		print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
	elif dxl_error != 0:
		print("%s" % packetHandler.getRxPacketError(dxl_error))
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	for i in range(1, len(ps)):
		...
		if i == 1 and ps[-1] is not None:
			...
			print("hsssss")
			print(xa,ya,za)
			if np.abs(xa) < 100 and np.abs(ya) < 100 and np.abs(za) < 100:
				clear = 1
			else:
				clear = 0
	...
	if begin == 1 and (theta[0]+theta[1]+theta[2]+theta[3]) != 0 :
		...
		position0 = [[80],[0],[0],[1]]
	position0[0][0] = position0[0][0] - 80
	print(position0)
